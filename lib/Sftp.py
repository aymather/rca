...
from .env import (RCA_NIELSEN_EMERGING_DAILY_SFTP_PASSWORD,
...
class Sftp:
    ...
    def delete(self, remote_fullfile):

        with self.connect() as sftp:
            sftp.remove(remote_fullfile)
            print(f'Deleted {remote_fullfile} successfully')
